...
** @param device_num Which device to initialise (valid for dual devices, always 0 for single
*devices)
** @return 0 on success, -1 on failure
*/
int mcp4531_init(mcp4531_handle_t* handle, I2C_HandleTypeDef* hi2c, uint8_t i2c_address,
                 uint8_t num_bits, uint8_t device_num) {
    ...
    HAL_StatusTypeDef status =
        HAL_I2C_Master_Transmit(handle->hi2c, handle->i2c_addr << 1, &command, 1, 2);
    if (status != HAL_OK) {
        handle->error = true;
    ...
    ** For MCP4531 prototypes the actual return value was 0x1F1
    ** For MCP4651 prototypes the actual return value was 0x1F0
    ** The reason for this is not known. */
    int ret = data[0] == 0x01 && (data[1] == 0xF0 || data[1] == 0xF1) ? 0 : -4;
    if (ret < 0) {
        handle->error = true;
    }
    return ret;
